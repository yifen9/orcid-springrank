...
SELECT
  lower(regexp_replace(coalesce(ascii_name,''),'\\s+',' ')) AS name_norm,
  country_code,
  geoname_id
...
UNION
SELECT
  lower(regexp_replace(trim(x.alt),'\\s+',' ')) AS name_norm,
  g.country_code,
  g.geoname_id
...
SELECT
  m.city_row_id,
  m.organization_city_raw,
  m.organization_city_norm,
  m.organization_country_iso2,
  m.pair_cnt,
  f.geoname_id,
  f.city_official_name,
  f.admin1_code,
  f.admin2_code,
  f.timezone,
  2 AS match_rule,
...
""",
    ...
    DuckDB.execute(
        db,
        "COPY audit TO '$(joinpath(audit_dir, "city_match_alias.parquet"))' WITH (FORMAT PARQUET, COMPRESSION 'zstd', OVERWRITE_OR_IGNORE TRUE)",
    )
    DuckDB.close(db)
    println("done")
end
