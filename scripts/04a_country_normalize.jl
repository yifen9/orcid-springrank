...
function norm_iso_strict(
    ...
    if length(s2) == 2 && all(isletter, s2)
        ...
        key = uppercase(replace(lowercase(Unicode.normalize(s, :NFKC)), r"\s+" => " "))
        key = strip(key)
        if haskey(terr, key)
            tgt = terr[key]
            return in(tgt, iso2) ? (tgt, "territorial") : (nothing, "fallback")
...
function main()
    ...
    DuckDB.execute(
        db,
        "CREATE TEMP TABLE orgraw AS SELECT organization_country AS raw_country, COUNT(*) AS cnt FROM read_parquet('$glob_pat') GROUP BY raw_country ORDER BY cnt DESC",
    )
    ...
    vals = DuckDB.execute(db, "SELECT raw_country, cnt FROM orgraw")
    DuckDB.execute(
        db,
        "CREATE TEMP TABLE cmap(raw_country VARCHAR, country_code VARCHAR, method VARCHAR, decided_at VARCHAR, cnt BIGINT)",
    )
    ...
    DuckDB.execute(
        db,
        "CREATE TEMP TABLE cmap2 AS SELECT raw_country,country_code,method,decided_at,cnt, ROW_NUMBER() OVER (ORDER BY COALESCE(country_code,'ZZ'), COALESCE(raw_country,'')) - 1 AS rn FROM cmap",
    )
    ...
    for (chunk_id, cnt) in groups
        ...
        DuckDB.execute(
            db,
            "COPY (SELECT raw_country,country_code,method,decided_at,cnt FROM cmap2 WHERE CAST(FLOOR(rn / $chunk_rows) AS BIGINT) = $chunk_id) TO '$out_file' WITH (FORMAT PARQUET, COMPRESSION 'zstd', OVERWRITE_OR_IGNORE TRUE)",
        )
        written += cnt
    ...
    DuckDB.execute(
        db,
        "CREATE TEMP TABLE audit AS SELECT SUM(cnt) AS total_rows, COUNT(*) AS distinct_raw_values, SUM(CASE WHEN country_code IS NOT NULL THEN cnt ELSE 0 END) AS mapped_rows, SUM(CASE WHEN country_code IS NULL THEN cnt ELSE 0 END) AS unmapped_rows, ROUND(100.0 * SUM(CASE WHEN country_code IS NOT NULL THEN cnt ELSE 0 END) / NULLIF(SUM(cnt),0), 2) AS mapped_pct, '$decided_at' AS decided_at FROM cmap",
    )
    DuckDB.execute(
        db,
