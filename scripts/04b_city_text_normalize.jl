...
SELECT organization_country AS raw_country,
       organization_city    AS raw_city,
       COUNT(*)             AS cnt
FROM read_parquet('$aff_glob')
GROUP BY raw_country, raw_city
""",
    )
...
SELECT raw_country, country_code
FROM read_parquet('$cmap_glob')
""",
    )
...
SELECT a.raw_country,
       c.country_code,
       a.raw_city,
       a.cnt
FROM aff a
LEFT JOIN cmap c USING(raw_country)
""",
    )
...
WITH base AS (
  SELECT
    ...
    country_code,
    cnt,
    regexp_replace(trim(COALESCE(raw_city,'')), '\\s+', ' ') AS trimmed
  FROM aff2
),
normed AS (
  SELECT
    ...
    country_code,
    cnt,
    NULLIF(lower(trimmed), '') AS norm_city
  FROM base
)
SELECT
  raw_city,
  ...
  ROW_NUMBER() OVER (ORDER BY COALESCE(country_code,'ZZ'), COALESCE(norm_city,''), COALESCE(raw_city,'')) - 1 AS rn
FROM normed
""",
    )
    ...
    qgc = DuckDB.execute(
        ...
        "SELECT CAST(FLOOR(rn / $chunk_rows) AS BIGINT) AS chunk_id, COUNT(*) AS cnt
         FROM citymap2 GROUP BY 1 ORDER BY 1",
    )
    ...
    for (chunk_id, cnt) in groups
        ...
        DuckDB.execute(
            ...
            """
COPY (
  SELECT raw_city, norm_city, country_code, decided_at, pair_cnt
  FROM citymap2
...
SELECT
  ...
  SUM(CASE WHEN norm_city IS NOT NULL THEN pair_cnt ELSE 0 END) AS normalized_weighted,
  ROUND(100.0 * SUM(CASE WHEN norm_city IS NOT NULL THEN pair_cnt ELSE 0 END) / NULLIF(SUM(pair_cnt),0), 2) AS normalized_pct,
  '$decided_at' AS decided_at
FROM citymap2
""",
    )
