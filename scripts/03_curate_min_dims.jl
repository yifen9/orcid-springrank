...
function fmt_bar(prefix::String, done::Int, total::Int, t0::Float64)
    ...
    @sprintf("%s %3d%%|%s| %d/%d  ETA %02d:%02d:%02d (%.2f s/unit)", prefix, pct, bar, done, total, h, m, ss, rate)
end
...
function main()
    ...
    print("\r", fmt_bar("materialize", tot, tot, t0)); println(); flush(stdout)
    ...
    for r in q_counts
        push!(groups, (year_dir=String(getproperty(r, :year_dir)),
    ...
    for g in groups
        out_dir = joinpath(out_root, "fact_affiliation", g.year_dir); mkpath(out_dir)
        ...
        """)
        done_rows += g.cnt
        print("\r", fmt_bar("write_fact", done_rows, tot, t0)); flush(stdout)
    end
    ...
    for r in q_pc
        push!(groups_p, (chunk_id=Int(getproperty(r, :chunk_id)),
                         cnt=Int(getproperty(r, :cnt))))
    end

    done_p = 0
    out_dir_p = joinpath(out_root, "dim_person"); mkpath(out_dir_p)
    for g in groups_p
        ...
        """)
        done_p += g.cnt
        print("\r", fmt_bar("write_person", done_p, tot_p, t0)); flush(stdout)
    end
    println(); flush(stdout)
    ...
    for r in q_oc
        push!(groups_o, (chunk_id=Int(getproperty(r, :chunk_id)),
                         cnt=Int(getproperty(r, :cnt))))
    end

    done_o = 0
    out_dir_o = joinpath(out_root, "dim_org_raw"); mkpath(out_dir_o)
    for g in groups_o
        ...
        """)
        done_o += g.cnt
        print("\r", fmt_bar("write_org", done_o, tot_o, t0)); flush(stdout)
    end
    println(); flush(stdout)
