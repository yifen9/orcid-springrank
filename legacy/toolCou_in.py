...
def generate_SpringRank_MAIN(RT,type,country,R,T,C,pathAdded):
    ...
    """
    for countryElement in country:
        generate_SpringRank_process(RT_FROMTO,None,countryElement,ID_TO_ROR,perDataRoleTitled,True,True,False,pathAdded)
        print('Done',countryElement)
    """
...
def generate_inequality_process(dataFlow,dataSpringRank,fileName1,fileName2):
    ORG_COUNTRIES = readCsv('source/country.csv')[0]

    dataOrgCou = []
    for x in tqdm(ORG_COUNTRIES): dataOrgCou.append(
        {'Country': x,
         'SpringRankUnadjusted': 0, 
         'CountUnadjusted': 0, 
         'SpringRankAdjusted': 0, 
         'CountAdjusted': 0, 
         'AverageUnadjusted': 0, 
         'AverageAdjusted': 0, 
         'VarianceUnadjusted': 0, 
         'VarianceAdjusted': 0}
         )

    lenDataFlow = len(dataFlow)
    lenDataSpringRank = len(dataSpringRank)

    """
    for x in tqdm(range(lenDataSpringRank)): dataSpringRank[x]['Count'] = 0

    for x in tqdm(range(lenDataFlow)):
        dataFlowX = dataFlow[x]
        dataFlowY_OriDisOrgID = dataFlowX['OriDisOrgID']
        dataFlowY_DesDisOrgID = dataFlowX['DesDisOrgID']

        for y in range(lenDataSpringRank):
            dataSpringRankY = dataSpringRank[y]
            dataSpringRankY_OrgID = dataSpringRankY['OrgID']

            if dataFlowY_OriDisOrgID == dataSpringRankY_OrgID or dataFlowY_DesDisOrgID == dataSpringRankY_OrgID:
                dataSpringRankY['Count'] += dataFlowX['Count']

    exportFile(dataSpringRank,fileName1)
    """

    for x in tqdm(range(lenDataSpringRank)):
        dataSpringRankX = dataSpringRank[x]

        i = ORG_COUNTRIES.index(dataSpringRankX['OrgLocationCountry'])

        dataOrgCouI = dataOrgCou[i]

        dataOrgCouI['SpringRankUnadjusted'] += dataSpringRankX['OrgSpringRank']
        dataOrgCouI['CountUnadjusted'] += 1

        dataOrgCouI['SpringRankAdjusted'] += (dataSpringRankX['Count'] * dataSpringRankX['OrgSpringRank'])
        dataOrgCouI['CountAdjusted'] += dataSpringRankX['Count']

    for x in tqdm(range(len(dataOrgCou))):
        if dataOrgCou[x]['CountUnadjusted'] > 0: 
            dataOrgCou[x]['AverageUnadjusted'] = dataOrgCou[x]['SpringRankUnadjusted'] / dataOrgCou[x]['CountUnadjusted']

        if dataOrgCou[x]['CountAdjusted'] > 0: 
            dataOrgCou[x]['AverageAdjusted'] = dataOrgCou[x]['SpringRankAdjusted'] / dataOrgCou[x]['CountAdjusted']
    ...
    for x in tqdm(range(lenDataSpringRank)):
        ...
        dataOrgCouI['VarianceUnadjusted'] += ((dataSpringRankX['OrgSpringRank']-dataOrgCouI['AverageUnadjusted']) * (dataSpringRankX['OrgSpringRank']-dataOrgCouI['AverageUnadjusted']))
        dataOrgCouI['VarianceAdjusted'] += (dataSpringRankX['Count']*((dataSpringRankX['OrgSpringRank']-dataOrgCouI['AverageAdjusted']) * (dataSpringRankX['OrgSpringRank']-dataOrgCouI['AverageAdjusted'])))

    for x in tqdm(range(len(dataOrgCou))):
        if dataOrgCou[x]['CountUnadjusted'] > 0: dataOrgCou[x]['VarianceUnadjusted'] /= (dataOrgCou[x]['CountUnadjusted']/100)
        if dataOrgCou[x]['CountAdjusted'] > 0: dataOrgCou[x]['VarianceAdjusted'] /= (dataOrgCou[x]['CountAdjusted']/100)

    exportFile(dataOrgCou,fileName2)

def generate_inequality_MAIN():
    pathOrg = 'data/organization/'
    pathOrgSpringRank = pathOrg + 'SpringRank/alltypes/allflows/allcountries/'

    dataFlow = readCsv_asDict(pathOrgSpringRank+'organization_flow_all.csv')
    dataSpringRank = readCsv_asDict(pathOrgSpringRank+'organization_SpringRank_all_count.csv')

    generate_inequality_process(dataFlow,dataSpringRank,pathOrgSpringRank+'organization_SpringRank_all_count.csv',pathOrgSpringRank+'organization_inequality.csv')

if __name__ == '__main__':
    generate_inequality_MAIN()

    # generate_SpringRank_process([['Ph.D','Position','N']],None,'US',None,None,False,True,False,'depted_cs')

    # generate_SpringRank_MAIN([],[],'All',True,True,False,'all')
